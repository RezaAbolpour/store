...
function OrderPage() {
        ...
        await getOredrById(item._id).then((res) => {
          if (res.data.data.orders.length != 0) {
            res.data.data.orders.forEach((item) => {
              console.log(item.deliveryStatus == false);
  ...
  return (
    <>
      ...
      ):<div className="text-red-500 flex justify-center items-center mt-5">سفارشی در حالت انتظار وجود ندارد</div>}
    </>
  );
}
